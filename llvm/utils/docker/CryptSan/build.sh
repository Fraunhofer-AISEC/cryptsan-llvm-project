...
if [ ! -d "$LINARO_DIR" ]; then
    echo "Cannot find linaro directory \"$LINARO_DIR\"! Abort"
    exit 1
fi

cd $LINARO_DIR
if [ ! -f "$LINARO_SYSROOT_FILENAME" ]; then
    ...
    if [[ $USE_DISTCC == "YES" ]]; then
        ...
        if [[ $BUILD_COMPILER_RT == "YES" ]]; then
            echo "cmake -S "$(pwd)/compiler-rt" -B "$BUILD_COMPILER_RT_DIR" \
                ...
                -DCMAKE_C_FLAGS="-O0 -fPIC --target=aarch64-linux-gnu -march=armv8.3-a -I$LINARO_DIR/sysroot-glibc-linaro-2.25-2018.05-aarch64-linux-gnu -B$LINARO_DIR/gcc-linaro-7.3.1-2018.05-x86_64_aarch64-linux-gnu --sysroot=$LINARO_DIR/sysroot-glibc-linaro-2.25-2018.05-aarch64-linux-gnu"            \
                -DCMAKE_CXX_FLAGS="-O0 -fPIC --target=aarch64-linux-gnu -march=armv8.3-a -I$LINARO_DIR/sysroot-glibc-linaro-2.25-2018.05-aarch64-linux-gnu -B$LINARO_DIR/gcc-linaro-7.3.1-2018.05-x86_64_aarch64-linux-gnu --sysroot=$LINARO_DIR/sysroot-glibc-linaro-2.25-2018.05-aarch64-linux-gnu""
